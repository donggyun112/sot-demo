...
test("a markdown file starts a document and joins a conversation", async ({
  ...
  try {
    ...
    await alice.getByRole("button", { name: "Workspaces" }).click();
    await alice
      .getByRole("menuitem", { name: "Alice Workspace", exact: true })
      .click();
    await alice.waitForURL(/\/w\/[^/]+$/);
    const aw = new URL(alice.url()).pathname.split("/")[2];
